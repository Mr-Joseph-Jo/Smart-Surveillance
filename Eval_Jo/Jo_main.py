...
def plot_cmc_comparison(experiments: List[Dict], out_path: Path):
    if not experiments:
        return
    plt.figure(figsize=(8, 5))
    plotted = False
    for res in experiments:
        cmc_vals = res.get('cmc')
        if not cmc_vals:
            continue
        ranks = np.arange(1, len(cmc_vals) + 1)
        plt.plot(ranks, np.array(cmc_vals) * 100, marker='o', label=res['name'])
        plotted = True
    if not plotted:
        plt.close()
        return
    ...
    plt.title('CMC Comparison Across Experiments')
...
def plot_ablation_study(experiments: List[Dict], out_path: Path):
    if not experiments:
        return
    names = [res['name'] for res in experiments]
    rank1_scores = [res['r1'] * 100 for res in experiments]
    map_scores = [res['map'] * 100 for res in experiments]
    x = np.arange(len(names))
    width = 0.35
    plt.figure(figsize=(10, 5))
    plt.bar(x - width / 2, rank1_scores, width, label='Rank-1')
    plt.bar(x + width / 2, map_scores, width, label='mAP')
...
class MarketEvaluator:
    ...
    def run(self, system, name, feature_bank=None):
        print(f"\n--- Experiment: {name} ---")
        ...
        for s in tqdm(self.gallery): 
            if feature_bank is not None:
                f = feature_bank['gallery'].get(s.image_path)
            else:
        ...
        for s in tqdm(self.query): 
            if feature_bank is not None:
                f = feature_bank['query'].get(s.image_path)
            else:
                img = cv2.imread(s.image_path)
                f = system.extract(img)
            if not f or not any(v is not None for v in f.values()):
        ...
        max_rank = max(1, min(20, len(gal_feats)))
        cmc = np.zeros(max_rank, dtype=np.float32)
        if results:
            for r in results:
                for rank, match in enumerate(r[:max_rank]):
                    if match['match']:
                        cmc[rank:] += 1
                        break
            cmc_curve = (cmc / len(results)).tolist()
        else:
            cmc_curve = cmc.tolist()
        
        return {'name': name, 'r1': r1, 'map': np.mean(aps), 'cmc': cmc_curve}
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DATASET_PATH):
        ...
        cache_path = Path("market_features.pkl")
        feature_bank = evaluator.load_cache(cache_path)

        if feature_bank is None:
            print("No cached features found. Building maximal feature bank...")
            master_system = MultiModalReID(['body', 'pose', 'face', 'hair'])
            feature_bank = {'gallery': {}, 'query': {}}
            for split_name, samples in [('gallery', evaluator.gallery), ('query', evaluator.query)]:
                for sample in tqdm(samples, desc=f"Caching {split_name.title()}"):
                    img = cv2.imread(sample.image_path)
                    if img is None:
                        continue
                    feats = master_system.extract(img)
                    feature_bank[split_name][sample.image_path] = feats
            evaluator.save_cache(feature_bank, cache_path)
        ...
        # 1. Segmented Body Only
        r1 = evaluator.run(body_only_system, "Segmented Body Only", feature_bank=feature_bank)
        
        # 2. Body + Face (Gated)
        r2 = evaluator.run(system, "Body + Face (Gated)", feature_bank=feature_bank)
        
        # 3. Body + Pose + Face
        r3 = evaluator.run(body_pose_face_system, "Body + Pose + Face", feature_bank=feature_bank)
        
        # 4. Full Gated Hybrid
        r4 = evaluator.run(full_system, "Full Gated Hybrid", feature_bank=feature_bank)
        ...
        plot_cmc_comparison(experiments, Path("debug_results") / "cmc_comparison.png")
        plot_ablation_study(experiments, Path("debug_results") / "ablation_study.png")
